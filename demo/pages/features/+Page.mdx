...
Using `choice=choice-name group=group-name` meta to group code blocks into multiple choices:
> The `group=group-name` meta is only required for the first code block.

```shell choice=hono group=server
...
Using `:::CodeGroup{#choice-name group=group-name} {:mdx}` directive to combine multiple contents for a single choice:
> The `group=group-name` attribute is only required for the first directive.

:::CodeGroup{#vike-react group=ui-ext}
```tsx
