...
- Using `// @detype-rename dummyVarName varName` above the affected line.
- Using `//~` at the beginning of the affected line.

```ts
const hello: string = 'world'// [!code --]
// @detype-rename dummyhello hello
const dummyhello: string[] = ['hello', 'world']// [!code ++]
//~console.log(hello)// [!code --]
console.log(hello[0])// [!code ++]
```

Some inline code block (without `ts-only`):
```ts inline
type S1 = string
```

Some inline code block (with `ts-only`):
```ts ts-only inline
type S2 = boolean
```
