...
```shell choice=hono persist-id=server-framework
...
Using `:::CodeGroup{choice=choice-name} {:mdx}` directive to combine multiple contents for a single choice:

:::CodeGroup{choice="vike-react" persist-id=frontend-framework}
```tsx
